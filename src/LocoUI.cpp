...
LocoUI::LocoUI(TFT_eSPI *tft, Tasks *tasks, DCCExCS *dccExCS, uint16_t address) : UI(tft, tasks), _dccExCS(dccExCS), _loco(address) {
...
void LocoUI::touch(uint8_t count, GTPoint* points) {
  // TODO, disallow buttons if CS isn't connected?
...
void LocoUI::release(uint8_t count, GTPoint* points) {
